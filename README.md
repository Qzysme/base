...
- RASS-Place augments GiFt with risk-aware adaptive spectral shaping. Enable it with `rass_place_flag = 1` together with `gift_init_flag`. Tunable knobs include:
  - `rass_risk_weight` / `rass_risk_threshold` to control the hinge penalty added to the DREAMPlace objective.
  - `rass_route_weight`, `rass_pin_weight`, `rass_multiscale_weight` for blending congestion and pin-utilization maps into the risk field.
  - `rass_adapt_flag`, `rass_num_samples`, `rass_hpwl_guard`, `rass_disp_guard_avg`, `rass_disp_guard_max` for adaptive spectral mixing during GiFt initialization.
  - `rass_refresh_interval`, `rass_feedback_flag` to toggle optional feedback modules (risk refresh and scheduled risk ramps).
Leaving `rass_place_flag` at `0` preserves the legacy DREAMPlace behaviour.
