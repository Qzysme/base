...
class NonLinearPlace(BasicPlace.BasicPlace):
    ...
    def _compute_route_metrics(self, params, placedb, pos_tensor):
        op = getattr(self.op_collections, "route_utilization_map_op", None)
        if op is None:
            try:
                self.op_collections.route_utilization_map_op = self.build_route_utilization_map(
                    params, placedb, self.data_collections
                )
                op = self.op_collections.route_utilization_map_op
            except Exception:
                return None
        try:
