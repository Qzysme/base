...
class NonLinearPlace(BasicPlace.BasicPlace):
    ...
    def __call__(self, params, placedb, learning_rate_value):
        ...
        if params.global_place_flag:
            ...
            for cur_stage, global_place_params in enumerate(global_place_stages):
                ...
                if not hasattr(self, "_initial_movable_pos"):
                    self._initial_movable_pos = self.pos[0].data.clone()
        ...
        self._log_final_metrics(params, placedb)
        return all_metrics

    def _log_final_metrics(self, params, placedb):
        if not hasattr(self, "pos") or not self.pos:
            return
        with torch.no_grad():
            current = self.pos[0].data.clone()
            if hasattr(self, "_initial_movable_pos"):
                initial = self._initial_movable_pos.to(current.device, current.dtype)
            else:
            ...
            cur_x = current[:num_movable]
            cur_y = current[num_nodes:num_nodes + num_movable]
            init_x = initial[:num_movable]
            init_y = initial[num_nodes:num_nodes + num_movable]

            disp = torch.sqrt((cur_x - init_x) ** 2 + (cur_y - init_y) ** 2)
            avg_disp = disp.mean().item() if disp.numel() > 0 else 0.0
            max_disp = disp.max().item() if disp.numel() > 0 else 0.0

            pin_metrics = self._compute_pin_metrics(params, placedb, current)
            route_metrics = self._compute_route_metrics(params, placedb, current)

        logging.info(
            "Final summary -> Average displacement: %.4f, Maximum displacement: %.4f",
            avg_disp,
            max_disp,
        )
        if pin_metrics:
    ...
    def _compute_pin_metrics(self, params, placedb, pos_tensor):
        op = self._ensure_pin_utilization_op(params, placedb, pos_tensor.device)
        if op is None:
            return None
        try:
            pin_map = op(pos_tensor).detach()
        except Exception:
            return None
    ...
    def _compute_route_metrics(self, params, placedb, pos_tensor):
        op = self._ensure_route_utilization_op(params, placedb, pos_tensor.device)
        if op is None:
            return None
        try:
            route_map = op(pos_tensor).detach()
        except Exception:
            return None
    ...
    def _ensure_pin_utilization_op(self, params, placedb, device):
        if getattr(self.op_collections, "pin_utilization_map_op", None) is not None:
            return self.op_collections.pin_utilization_map_op
        if getattr(self, "_pin_util_builder_failed", False):
            return None
        if not placedb.num_routing_grids_x or not placedb.num_routing_grids_y:
            self._pin_util_builder_failed = True
            return None
        try:
            op = pin_utilization.PinUtilization(
                pin_weights=self.data_collections.pin_weights,
                flat_node2pin_start_map=self.data_collections.flat_node2pin_start_map,
                ...
                unit_pin_capacity=self.data_collections.unit_pin_capacity,
                pin_stretch_ratio=params.pin_stretch_ratio,
                deterministic_flag=params.deterministic_flag,
            )
            self.op_collections.pin_utilization_map_op = op
            return op
        except Exception:
            self._pin_util_builder_failed = True
            return None

    def _ensure_route_utilization_op(self, params, placedb, device):
        if getattr(self.op_collections, "route_utilization_map_op", None) is not None:
            return self.op_collections.route_utilization_map_op
        if getattr(self, "_route_util_builder_failed", False):
            return None
        if not placedb.num_routing_grids_x or not placedb.num_routing_grids_y:
            self._route_util_builder_failed = True
            return None
        try:
            op = rudy.Rudy(
                ...
                initial_horizontal_utilization_map=getattr(
                    self.data_collections, "initial_horizontal_utilization_map", None
                ),
                initial_vertical_utilization_map=getattr(
                    self.data_collections, "initial_vertical_utilization_map", None
                ),
                deterministic_flag=params.deterministic_flag,
            )
            self.op_collections.route_utilization_map_op = op
            return op
        except Exception:
            self._route_util_builder_failed = True
            return None
